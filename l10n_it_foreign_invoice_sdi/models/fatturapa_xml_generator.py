...
class FatturaPAXmlGenerator(models.AbstractModel):
    ...
    def _add_body(
        ...
        # DatiFattureCollegate (fattura originale fornitore)
        if invoice.invoice_number:
            dfc = etree.SubElement(dg, '{%s}DatiFattureCollegate' % FATTURAPA_NS)
            etree.SubElement(
        ...
        if invoice.line_ids:
            for idx, line in enumerate(invoice.line_ids, 1):
                self._add_dettaglio_linea(
                    dbs, idx, line.description or invoice.description_type or '/',
    ...
    def _validate_xsd(self, root: etree._Element) -> None:
        ...
        Se lo schema XSD non è disponibile localmente, logga un warning
        e continua (non blocca la generazione).
        """
        import os
        ...
        try:
            ...
            if not schema.validate(root):
                errors = '\n'.join(
                    str(e) for e in schema.error_log
                )
