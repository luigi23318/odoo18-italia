...
class ArubaSdiService(models.AbstractModel):
    ...
    def _authenticate(self, config: dict) -> str:
        """Autenticazione Aruba Premium API.

        POST /auth/signin → token JWT
        """
    ...
    def send_invoice(self, invoice: 'foreign.invoice.import') -> None:
        """Invia XML FatturaPA allo SDI via Aruba Premium.
    ...
    def check_notification(self, invoice: 'foreign.invoice.import') -> None:
        """Verifica notifiche SDI per una fattura.
    ...
    def get_multicedenti(self) -> list:
        """Ottiene lista cedenti per account multicedente.

        GET /auth/multicedenti
        """
        config = self._get_config()
        token = self._authenticate(config)
        resp = requests.get(
    ...
    def switch_account(self, vat: str) -> None:
        """Switch contesto P.IVA per multicedente.

        POST /auth/switchAccount
        """
        config = self._get_config()
        token = self._authenticate(config)
        resp = requests.post(
