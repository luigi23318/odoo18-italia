...
class ResCompany(models.Model):
    ...
    def _get_pec_sdi_destination(self):
        ...
        self.ensure_one()
        if self.l10n_it_edi_pec_mode == 'test':
            if not self.l10n_it_pec_sdi_test_address:
                raise ValidationError(
                    _("Modalità Test: configurare l'indirizzo PEC SDI di test "
                      "prima di inviare. Senza, l'invio andrebbe allo SDI di produzione.")
                )
    ...
    def _check_pec_configuration(self):
        ...
        if self.l10n_it_edi_pec_mode in ('test', 'production'):
            ...
            # Warning IMAP: senza, le notifiche SDI non verranno mai ricevute
            if not self.l10n_it_pec_imap_server:
                _logger.warning(
                    "Company %s: IMAP PEC non configurato. "
                    "Le notifiche SDI (RC, NS, MC, ecc.) non verranno ricevute "
